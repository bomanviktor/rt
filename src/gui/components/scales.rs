...
pub fn horizontal_scale(label: &str, adjustment: Adjustment, vertical_box: &Box) -> Scale {
    let sample_size_label = gtk::Label::new(Some(label));
    vertical_box.pack_start(&sample_size_label, false, false, 0);
    ...
    vertical_box.pack_start(&sample_size_scale, false, true, 0);
    sample_size_scale
}
